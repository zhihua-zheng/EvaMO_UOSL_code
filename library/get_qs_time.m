...
SKF.Wstar = nthroot(-BLD.*SKF.B0,3);

Uble = max([SKF.Wstar SKF.Ustar],[],2,'includenan');
Tturnover = BLD./Uble/3600; % [hour]

itime_mid = (itime(1:end-1) + itime(2:end))/2;

% buoyancy forcing time scale
dB0_dt = center_diff(SKF.B0,itime,1,'mid');
dB0_dtl = [nan; dB0_dt];
dBo_dtr = [dB0_dt; nan];
TB0l = abs(SKF.B0 ./ dB0_dtl)*24; % [hour]
TB0r = abs(SKF.B0 ./ dBo_dtr)*24; % [hour]
TB0  = mean([TB0l TB0r],2,'includenan');
...
Texf = min([TB0 Twstr],[],2,'includenan');
...
axes(hax(1))
yyaxis left
plot(idatm_loc(Jchunk),-SKF.B0(Jchunk),'-k','linewidth',1.5); hold on
...
ylh1 = ylabel('-B_0 [m^2 s^{-3}]','fontsize',18,'Interpreter','tex');
ylh1.Units = 'Inches';
